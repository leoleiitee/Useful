...
User Function AdvplExec()
	...
	DEFINE MSDIALOG oDlg FROM 0,0 TO 400,380 PIXEL TITLE "A D V P L    E X E C"
	DEFINE FONT oFnt NAME "Arial" Size 6,22

	oSay:= tSay():New(10,10,{||"Função | Fonte.PRW | INFO"},oDlg,,oFnt,,,,.T.)
	oMemo:= tMultiget():New(20,10,{|u|if(Pcount()>0,aFunc[pcount()+1]:=u,aFunc[1])},oDlg,170,70,,,,,,.T.)

	oSay:= tSay():New(095,10,{||"Retorno"},oDlg,,oFnt,,,,.T.)
	@ 105,10 MSGET oSay VAR bRet SIZE 170,50 OF oDlg PIXEL


	@ 170,010 BUTTON oButton PROMPT "OK(S/Err)"  OF oDlg PIXEL SIZE 40,15 ACTION {||bRet:=Exc("") , bFlag:=.t., oDlg:Refresh() }
	@ 170,050 BUTTON oButton PROMPT "OK(C/Err)"  OF oDlg PIXEL SIZE 40,15 ACTION {||bRet:=ExcErr("") , bFlag:=.t., oDlg:Refresh() }
